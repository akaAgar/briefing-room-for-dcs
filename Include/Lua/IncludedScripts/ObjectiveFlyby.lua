briefingRoom.extensions.objectiveFlyby = {}
briefingRoom.extensions.objectiveFlyby.MAX_ALTITUDE_GROUND_RECON = 1524 -- in meters (5000 feet)
briefingRoom.extensions.objectiveFlyby.MAX_ALT_DISTANCE_AIR_RECON = 152 -- in meters (500 feet)
briefingRoom.extensions.objectiveFlyby.MAX_DISTANCE = 500 -- in meters

function briefingRoom.extensions.objectiveFlyby.checkFlyby(player, unit)
  ...
  if distance > briefingRoom.extensions.objectiveFlyby.MAX_DISTANCE then
    return false
  end
  ...
  if unitCategory == Unit.Category.AIRPLANE or unitCategory == Unit.Category.HELICOPTER then
    if math.abs(player:getPoint().z - unit:getPoint().z) < briefingRoom.extensions.objectiveFlyby.MAX_ALT_DISTANCE_AIR_RECON then
      return true
    end
  else
    if player:getPoint().z - land.getHeight <= briefingRoom.extensions.objectiveFlyby.MAX_ALTITUDE_GROUND_RECON then
      return true
    end
  end
...
function briefingRoom.extensions.objectiveFlyby.update(args, time)
    ...
    if objective.status == brMissionStatus.IN_PROGRESS then
      for _,p in ipairs(players) do
        for __,u in ipairs(objective.unitsID) do
          if briefingRoom.extensions.objectiveFlyby.checkFlyby(p, dcsExtensions.getUnitByID(u)) then
...
timer.scheduleFunction(briefingRoom.extensions.objectiveFlyby.update, nil, timer.getTime() + 1)
