...
function briefingRoom.mission.coreFunctions.completeObjective(index)
  ...
  if briefingRoom.mission.objectivesLeft <= 0 then
    ...
    -- if briefingRoom.mission.parameters.endMode == -2 then
      -- missionCommands.addCommandForCoalition($LUAPLAYERCOALITION$, "End mission now", nil, briefingRoom.mission.functions.endMissionIn, 0)
...
missionCommands.addCommandForCoalition($LUAPLAYERCOALITION$, "Mission status", nil, briefingRoom.f10MenuCommands.missionStatus, nil)

-- Mission features menu
briefingRoom.f10Menu.missionFeatures = missionCommands.addSubMenuForCoalition($LUAPLAYERCOALITION$, "Support", nil)
