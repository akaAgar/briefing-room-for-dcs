...
function briefingRoom.mission.missionFeatures.friendlyTaskableCAS.eventHandler:onEvent(event)
  if event.id == world.event.S_EVENT_MARK_REMOVED then
    if briefingRoom.mission.missionFeatures.friendlyTaskableCAS.markID ~= nil and event.idx == briefingRoom.mission.missionFeatures.friendlyTaskableCAS.markID then
      if not briefingRoom.mission.missionFeatures.friendlyTaskableCAS.disableCooRemovedRadioMessage then
        briefingRoom.radioManager.play("$LANG_CAS$: $LANG_DISCARDCOORDINATES$", "RadioCoordinatesDiscardedM")
      end
  ...
  elseif event.id == world.event.S_EVENT_MARK_ADDED then
    ...
    if markText == briefingRoom.mission.missionFeatures.friendlyTaskableCAS.MARKER_NAME then
      ...
      briefingRoom.radioManager.play("$LANG_CAS$: $LANG_UPDATECOORDINATES$", "RadioCoordinatesUpdatedM")
      return
    end
...
function briefingRoom.mission.missionFeatures.friendlyTaskableCAS.launchBombingRun()
  briefingRoom.radioManager.play("$LANG_PILOT$: $LANG_CASREQUEST$", "RadioPilotBeginYourAttack")
  ...
  for _,m in ipairs(marks) do
    if briefingRoom.mission.missionFeatures.friendlyTaskableCAS.markID ~= nil and m.idx == briefingRoom.mission.missionFeatures.friendlyTaskableCAS.markID then
      ...
      if group ~= nil then
        ...
        briefingRoom.radioManager.play("$LANG_CAS$: $LANG_BEGINATTACK$", "RadioOtherPilotBeginAttack", briefingRoom.radioManager.getAnswerDelay(), nil, nil)
      end
      return
    end
  end

  briefingRoom.radioManager.play("$LANG_CAS$: $LANG_CASNOCOORDINATES$", "RadioArtilleryNoCoordinates", briefingRoom.radioManager.getAnswerDelay())
end
...
missionCommands.addCommandForCoalition($LUAPLAYERCOALITION$, "$LANG_CASMENU$", briefingRoom.f10Menu.missionMenu, briefingRoom.mission.missionFeatures.friendlyTaskableCAS.launchBombingRun, nil)
