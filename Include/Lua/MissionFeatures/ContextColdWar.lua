...
function briefingRoom.mission.missionFeatures.contextColdWar.eventHandler:onEvent(event)
  ...
  if event.id ~= world.event.S_EVENT_DEAD and event.id ~= world.event.S_EVENT_CRASH and event.id ~= world.event.S_EVENT_HIT and event.id ~= world.event.S_EVENT_SHOT then return end
  ...
  briefingRoom.radioManager.play("Command: Be advised, enemy is aware of hostile activity in its airspace and may engage you at any moment.", "RadioHQColdWarGoneHot")
