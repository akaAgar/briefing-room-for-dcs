...
function briefingRoom.mission.missionFeatures.ImprovementResponsiveAircraftActivator.detectedSpawner(args, time)
    ...
    if briefingRoom.mission.hasStarted and briefingRoom.mission.missionFeatures.ImprovementResponsiveAircraftActivator.isPlayerDetected() and not briefingRoom.mission.missionFeatures.ImprovementResponsiveAircraftActivator.isCASInAir() then
...
function briefingRoom.mission.missionFeatures.ImprovementResponsiveAircraftActivator.isCASInAir()
    ...
    for _,g in pairs(coalition.getGroups($LUAENEMYCOALITION$), Group.Category.AIRPLANE) do
        ...
        if dcsExtensions.getAliveUnitInGroup(g:getID())and #groupRouteData[1].task.params.tasks > 0 and groupRouteData[1].task.params.tasks[1].key == "CAP" then
...
function briefingRoom.mission.missionFeatures.ImprovementResponsiveAircraftActivator.isPlayerDetected()
    ...
    for _, player in ipairs(dcsExtensions.getAllPlayers()) do
        for _,g in pairs(coalition.getGroups($LUAENEMYCOALITION$)) do
            if dcsExtensions.getAliveUnitInGroup(g:getID()) then
...
timer.scheduleFunction(briefingRoom.mission.missionFeatures.ImprovementResponsiveAircraftActivator.detectedSpawner, nil, timer.getTime() + briefingRoom.mission.missionFeatures.ImprovementResponsiveAircraftActivator.interval)