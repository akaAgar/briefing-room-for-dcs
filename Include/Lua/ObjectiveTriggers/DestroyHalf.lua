...
briefingRoom.mission.objectiveTriggers[$OBJECTIVEINDEX$] = function(event)
  ...
  local maxAmountLeft = math.clamp(math.ceil(briefingRoom.mission.objectives[$OBJECTIVEINDEX$].unitsCount / 2), 1, briefingRoom.mission.objectives[$OBJECTIVEINDEX$].unitsCount - 1)
  -- Mark the objective as complete if 50% of targets have been destroyed
  if #briefingRoom.mission.objectives[$OBJECTIVEINDEX$].unitsID < maxAmountLeft) then -- 50% of target units destroyed, objective complete
