      ...
      if unit ~= nil then
        ...
        if distance < 9260 and math.abs(vec2p.y - vec2u.y) < 2438 then -- less than 5nm away on the X/Z axis, less than 8000 feet of altitude difference
          if math.random(1, 2) == 1 then
            briefingRoom.radioManager.play("Pilot: Command, I have a good visual on target.", "RadioPilotTargetReconned1")
          else
            briefingRoom.radioManager.play("Pilot: Command, positive visual on target.", "RadioPilotTargetReconned2")
          end
