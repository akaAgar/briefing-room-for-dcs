...
table.insert(briefingRoom.mission.objectiveTriggers, function(event)
    ...
    if event.id == world.event.S_EVENT_DEAD or event.id == world.event.S_EVENT_CRASH or event.id == world.event.S_EVENT_AI_ABORT_MISSION then destructionEvent = true end
  ...
  do
    ...
    for _,i in ipairs(briefingRoom.mission.objectives[$OBJECTIVEINDEX$].unitNames) do
      ...
      if unit ~= nil and not unit:hasAttribute("Air Defence") then
