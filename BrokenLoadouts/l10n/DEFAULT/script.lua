...
function briefingRoom.radioManager.doRadioMessage(args, time)
  if args.message ~= nil then -- a message was provided, print it
    ...
    trigger.action.outTextForCoalition(coalition.side.BLUE, args.message, duration, false)
  end
  ...
  if args.oggFile ~= nil and briefingRoom.radioManager.enableAudioMessages then -- a sound was provided and radio sounds are enabled, play it
    trigger.action.outSoundForCoalition(coalition.side.BLUE, args.oggFile..".ogg")
  else -- else play the default sound
    trigger.action.outSoundForCoalition(coalition.side.BLUE, "Radio0.ogg")
  end
...
briefingRoom.aircraftActivator.currentQueue = {  } -- current queue of aircraft group IDs to spawn every INTERVAL seconds
briefingRoom.aircraftActivator.reserveQueue = { 2,3,4,5,6,7,8, } -- additional aircraft group IDs to be added to the queue later
...
function briefingRoom.handleGeneralKill(event) 
  if event.id == world.event.S_EVENT_DEAD or event.id == world.event.S_EVENT_CRASH then
    ...
    if event.initiator:getCoalition() ~= coalition.side.BLUE then -- unit is an enemy, radio some variation of a "enemy destroyed" message
...
function briefingRoom.mission.coreFunctions.completeObjective(index)
  ...
  if briefingRoom.f10Menu.objectives[index] ~= nil then
    missionCommands.removeItemForCoalition(coalition.side.BLUE, briefingRoom.f10Menu.objectives[index])
...
briefingRoom.f10Menu.missionMenu = missionCommands.addSubMenuForCoalition(coalition.side.BLUE, "Mission", nil)
missionCommands.addCommandForCoalition(coalition.side.BLUE, "Mission status", briefingRoom.f10Menu.missionMenu, briefingRoom.f10MenuCommands.missionStatus, nil)
...
briefingRoom.mission.objectives[1] = { complete = false, groupID = 1, hideTargetCount = false, name = "SWAP", targetCategory = Unit.Category.GROUND_UNIT, task = "Destroy the APC designated objective SWAP.", unitsCount = 1, unitsID = { 1 } }
briefingRoom.f10Menu.objectives[1] = missionCommands.addSubMenuForCoalition(coalition.side.BLUE, "Objective SWAP", nil)
briefingRoom.mission.objectives[1].waypoint = { ["x"] = -192600.8069647341, ["y"] = 559436.1030741014 }

briefingRoom.mission.objectivesLeft = #briefingRoom.mission.objectives -- Store the total of objective left to complete

for i=1,#briefingRoom.mission.objectives do
  missionCommands.addCommandForCoalition(coalition.side.BLUE, "Request waypoint coordinates", briefingRoom.f10Menu.objectives[i], briefingRoom.f10MenuCommands.getWaypointCoordinates, i)
end
...
-- Triggers the completion of objective 1 when all targets are destroyed
briefingRoom.mission.objectiveTriggers[1] = function(event)
    -- Mission complete, nothing to do
    if briefingRoom.mission.complete then return false end
  
    -- Objective complete, nothing to do
    if briefingRoom.mission.objectives[1].complete then return false end
    ...
    if briefingRoom.mission.objectives[1].targetCategory == Unit.Category.HELICOPTER and event.id == world.event.S_EVENT_LAND then destructionEvent = true end
    ...
    if not table.contains(briefingRoom.mission.objectives[1].unitsID, unitID) then return false end
  
    -- Remove the unit from the list of targets
    table.removeValue(briefingRoom.mission.objectives[1].unitsID, unitID)
    ...
    if #briefingRoom.mission.objectives[1].unitsID < 1 then -- all target units destroyed, objective complete
      briefingRoom.mission.coreFunctions.completeObjective(1)
    end
  ...
  do
    ...
    for _,i in ipairs(briefingRoom.mission.objectives[1].unitsID) do
    ...
    if #newTargetTable > 0 then
      briefingRoom.mission.objectives[1].unitsID = newTargetTable
      briefingRoom.mission.objectives[1].unitsCount = #newTargetTable
    end
  end
...
briefingRoom.mission.objectiveFeatures[1].targetDesignationCoordinates = function()
  briefingRoom.radioManager.play("Require update on target coordinates.", "RadioPilotTargetCoordinates")
    
  local unit = dcsExtensions.getAliveUnitInGroup(briefingRoom.mission.objectives[1].groupID)
    
  if #briefingRoom.mission.objectives[1].unitsID == 0 then -- no target units left
  ...
  local unit = dcsExtensions.getUnitByID(briefingRoom.mission.objectives[1].unitsID[1])
  if unit == nil then -- no unit found with the ID, try searching for a static
    unit = dcsExtensions.getStaticByID(briefingRoom.mission.objectives[1].unitsID[1])
...
missionCommands.addCommandForCoalition(coalition.side.BLUE, "Require target coordinates", briefingRoom.f10Menu.objectives[1], briefingRoom.mission.objectiveFeatures[1].targetDesignationCoordinates)

briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser = { }
briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserSpot = nil -- current laser spot
briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserTarget = nil -- current lased target
    ...
    if briefingRoom.mission.objectiveFeatures[i].targetDesignationLaser ~= nil then
      ...
      if briefingRoom.mission.objectiveFeatures[i].targetDesignationLaser.laserTarget ~= nil then
        ...
        if targetDestroyed then -- target is dead
          ...
          if briefingRoom.mission.objectiveFeatures[i].targetDesignationLaser.laserSpot == nil then
            briefingRoom.mission.objectiveFeatures[i].targetDesignationLaser.laserSpot = Spot.createLaser(briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserTarget, { x = 0, y = 2.0, z = 0 }, targetPos, LASER_CODE)
...
briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.turnOn = function()
  briefingRoom.radioManager.play("Can you paint the target for me?", "RadioPilotLaseTarget")

  -- already lasing something
  if briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserTarget ~= nil then
  ...
  if #briefingRoom.mission.objectives[1].unitsID == 0 then
    briefingRoom.radioManager.play("Negative, no visual on any target.", "RadioSupportNoTarget", briefingRoom.radioManager.getAnswerDelay())
    return
  end
  ...
  briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserTarget = unit
end

-- Stops lasing the target
briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.turnOff = function()
  ...
  -- destroy laser spot, if any
  if briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserSpot ~= nil then
    Spot.destroy(briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserSpot)
    briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserSpot = nil
  end

  -- unset target and play radio message
  briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.laserTarget = nil
...
missionCommands.addCommandForCoalition(coalition.side.BLUE, "Designate target with laser", briefingRoom.f10Menu.objectives[1], briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.turnOn)
missionCommands.addCommandForCoalition(coalition.side.BLUE, "Stop lasing target", briefingRoom.f10Menu.objectives[1], briefingRoom.mission.objectiveFeatures[1].targetDesignationLaser.turnOff)

-- Time (in seconds) until next smoke is available
briefingRoom.mission.objectiveFeatures[1].targetDesignationSmokeMarkerNextSmoke = -999999

-- Spawn smoke marker
briefingRoom.mission.objectiveFeatures[1].targetDesignationSmokeMarkerDoSmoke = function(args)
  trigger.action.smoke(args.position, args.color)
  return nil
end

-- "Mark target with smoke" F10 radio command
briefingRoom.mission.objectiveFeatures[1].targetDesignationSmokeMarker = function()
  ...
  if timeNow < briefingRoom.mission.objectiveFeatures[1].targetDesignationSmokeMarkerNextSmoke then -- Smoke not ready
  ...
  briefingRoom.mission.objectiveFeatures[1].targetDesignationSmokeMarkerNextSmoke = timeNow + SMOKE_DURATION
  
  -- Play radio message and setup smoke creating function
  local args = { position = unit:getPoint(), color = trigger.smokeColor.Red }
  if unit:getCoalition() == coalition.side.BLUE then args.color = trigger.smokeColor.Green end
  briefingRoom.radioManager.play("Affirm, target marked with smoke.", "RadioSupportTargetMarkedWithSmoke", briefingRoom.radioManager.getAnswerDelay(), briefingRoom.mission.objectiveFeatures[1].targetDesignationSmokeMarkerDoSmoke, args)
end
      
-- Add the command to the F10 menu
missionCommands.addCommandForCoalition(coalition.side.BLUE, "Mark target with smoke", briefingRoom.f10Menu.objectives[1], briefingRoom.mission.objectiveFeatures[1].targetDesignationSmokeMarker)
